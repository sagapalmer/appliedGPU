...
int mover_PC_gpu(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param) {
    ...
    cudaMalloc(&(d_part->z), sizeof(FPpart) * max_number_of_particles);
    cudaMalloc(&(d_part->u), sizeof(FPpart) * max_number_of_particles);
    cudaMalloc(&(d_part->v), sizeof(FPpart) * max_number_of_particles);
    cudaMalloc(&(d_part->w), sizeof(FPpart) * max_number_of_particles);
    cudaMalloc(&(d_part->q), sizeof(FPinterp) * max_number_of_particles);
    ...
    mover_PC_gpu_kernel<<<gridDim, blockDim>>>(d_part, field, grd, param);
