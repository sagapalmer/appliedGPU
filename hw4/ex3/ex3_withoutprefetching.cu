...
int main(int argc, char **argv) {
  ...
  //cusparseMatDescr_t Adescriptor; OLD VERSION???   // Mat descriptor needed by cuSPARSE
  cusparseSpMatDescr_t Adescriptor;
  cusparseDnVecDescr_t tempDense, tmpDense;
  ...
  cusparseCheck(cusparseCreateDnVec(&tempDense, dimX, temp, CUDA_R_64F));
  cusparseCheck(cusparseCreateDnVec(&tmpDense, dimX, tmp, CUDA_R_64F));
  ...
  cusparseCheck(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
  ...
  cusparseCheck(cusparseDestroyDnVec(tempDense));
  cusparseCheck(cusparseDestroyDnVec(tmpDense));
